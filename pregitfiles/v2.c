...
*Function: copy_huge
*-------------------
*copy huge from source to target
*
*   target: pointer of huge_t array
*	target_len: pointer to length of huge_t 
*   source: pointer of huge_t array to be added to target
...
void power_huge(huge_t *target, int *target_len, huge_t *exp, int exp_len){
	huge_t huge_i; /*huge incrementer*/
	int huge_i_len=0;
	set_huge_to_zero(&huge_i, &huge_i_len);
	inc_huge(&huge_i, &huge_i_len); /*increases by one because x^2 requires 1
	                                  multiplication not 2*/
	

	huge_t base;
	int base_len = *target_len; /*stores copy of target to repeatedly multiply*/
	copy_huge(&base, &base_len, target, *target_len);	

	while(!(huges_equal(&huge_i, huge_i_len, exp, exp_len))){
		mult_huge(target, target_len, &base, base_len);/*mults target by huge*/
...
void inc_huge(huge_t *target, int *target_len){
	...
	add_int_to_digit(target, target_len, 0,1); /*increments i_huge by 1*/
	trim_zeros(target, target_len); 
